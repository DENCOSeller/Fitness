...
import MeasurementsPanel from '@/components/body/measurements-panel';
...
type Tab = 'picooc' | 'measurements';

export default function BodyPage() {
  const [tab, setTab] = useState<Tab>('picooc');
  ...
  return (
    <div className="max-w-lg mx-auto p-4 space-y-4">
      ...
      <div className="flex bg-[#1C1C1E] rounded-xl p-1 gap-1">
        <button
          onClick={() => setTab('picooc')}
          className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${
            tab === 'picooc' ? 'bg-[#2C2C2E] text-white' : 'text-[#8E8E93]'
          }`}
        >
          Picooc / Весы
        </button>
        <button
          ...
          className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${
            tab === 'measurements' ? 'bg-[#2C2C2E] text-white' : 'text-[#8E8E93]'
          }`}
        >
          Замеры тела
        </button>
