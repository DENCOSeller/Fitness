      ...
      exercises: {
        ...
        items: {
          ...
          properties: {
            ...
            type: {
              type: 'string',
              enum: ['strength', 'bodyweight', 'cardio', 'timed'],
              description: 'Тип: strength (штанга/гантели), bodyweight (своё тело), cardio (бег/вело), timed (планка/растяжка)',
            },
            sets: { type: 'integer', description: 'Количество подходов' },
            reps: { type: 'integer', description: 'Повторения (для strength, bodyweight)' },
            weight_kg: { type: 'number', description: 'Вес в кг (для strength; для bodyweight — только утяжелитель)' },
            duration_min: { type: 'number', description: 'Длительность в минутах (для cardio, timed)' },
            speed_kmh: { type: 'number', description: 'Скорость км/ч (для cardio)' },
            distance_km: { type: 'number', description: 'Дистанция км (для cardio)' },
            incline_pct: { type: 'number', description: 'Наклон % (для cardio, беговая дорожка)' },
